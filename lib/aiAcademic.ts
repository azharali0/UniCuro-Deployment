...
export async function askAcademicAssistant(userId: string, prompt: string) {
  ...
  await prisma.aiMessage.create({ data: { conversationId: conversation.id, role: "USER", content: prompt } });
  ...
  await prisma.aiMessage.create({ data: { conversationId: conversation.id, role: "ASSISTANT", content } });
